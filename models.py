...
class DocumentAnalysis(db.Model):
    """Document analysis results from Azure and Gemini AI"""
    ...
    # Processing status
    processing_status = db.Column(db.String(20), default='pending', nullable=False)  # pending, processing, completed, error
    processing_start_time = db.Column(db.DateTime)
    processing_end_time = db.Column(db.DateTime)
    
    # Azure Document Intelligence results
    azure_doc_type = db.Column(db.String(100))
    azure_confidence = db.Column(db.Float)
    azure_fields = db.Column(db.JSON)  # Extracted fields from Azure
    azure_raw_response = db.Column(db.JSON)  # Full Azure response
    azure_processing_time = db.Column(db.Float)
    
    # Gemini AI analysis results
    gemini_document_category = db.Column(db.String(100))
    gemini_analysis_summary = db.Column(db.Text)
    gemini_extracted_info = db.Column(db.JSON)  # Key info extracted by Gemini
    gemini_bookmark_structure = db.Column(db.JSON)  # Hierarchical bookmark structure
    gemini_raw_response = db.Column(db.JSON)  # Full Gemini response
    gemini_processing_time = db.Column(db.Float)
    
    # Validation and comparison
    validation_errors = db.Column(db.JSON)  # List of validation errors
    field_comparison = db.Column(db.JSON)  # Comparison between Azure and Gemini results
    
    # PII handling
    pii_mode = db.Column(db.String(20), default='mask')  # ignore, mask, remove
    ...
    # Relationships
    client_document = db.relationship('ClientDocument', backref='analysis')
    firm = db.relationship('Firm', backref='document_analyses')
    ...
    @property
    def has_azure_results(self):
        """Check if Azure analysis was completed"""
        return self.azure_fields is not None and len(self.azure_fields) > 0
    
    @property
    def has_gemini_results(self):
        """Check if Gemini analysis was completed"""
        return self.gemini_extracted_info is not None and len(self.gemini_extracted_info) > 0
    ...
    def get_combined_fields(self):
        ...
        if self.azure_fields:
            combined.update(self.azure_fields)
        
        # Add/override with Gemini fields
        if self.gemini_extracted_info:
            combined.update(self.gemini_extracted_info)
        
        return combined


class WorkpaperBatch(db.Model):
    """Batch of documents for workpaper generation"""
    __tablename__ = 'workpaper_batch'
    ...
    description = db.Column(db.Text)
    tax_year = db.Column(db.String(4))  # e.g., "2024"
    preparer_name = db.Column(db.String(100))
    ...
    # Workpaper file information
    workpaper_filename = db.Column(db.String(255))
    workpaper_file_path = db.Column(db.String(500))
    workpaper_file_size = db.Column(db.Integer)
    workpaper_page_count = db.Column(db.Integer)
    ...
    # Relationships
    firm = db.relationship('Firm', backref='workpaper_batches')
    client = db.relationship('Client', backref='workpaper_batches')
    ...
    def progress_percentage(self):
        ...
        if self.total_documents == 0:
            return 0
        return round((self.processed_documents / self.total_documents) * 100)
    ...
    @property
    def workpaper_file_size_formatted(self):
        """Human-readable file size"""
        if not self.workpaper_file_size:
            return "0 B"
        
        if self.workpaper_file_size < 1024:
            return f"{self.workpaper_file_size} B"
        elif self.workpaper_file_size < 1024 * 1024:
            return f"{self.workpaper_file_size / 1024:.1f} KB"
        else:
            return f"{self.workpaper_file_size / (1024 * 1024):.1f} MB"


class BatchDocument(db.Model):
    """Documents included in a workpaper batch"""
    __tablename__ = 'batch_document'
    
    id = db.Column(db.Integer, primary_key=True)
    batch_id = db.Column(db.Integer, db.ForeignKey('workpaper_batch.id'), nullable=False)
    client_document_id = db.Column(db.Integer, db.ForeignKey('client_document.id'), nullable=False)
    document_analysis_id = db.Column(db.Integer, db.ForeignKey('document_analysis.id'), nullable=True)
    
    # Document categorization for workpaper organization
    category = db.Column(db.String(100))  # Income Documents, Deduction Documents, etc.
    document_type = db.Column(db.String(100))  # W-2, 1099, Receipt, etc.
    bookmark_path = db.Column(db.String(500))  # Full bookmark path in workpaper
    sort_order = db.Column(db.Integer, default=0)
    
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    
    # Relationships
    batch = db.relationship('WorkpaperBatch', backref='batch_documents')
    client_document = db.relationship('ClientDocument', backref='batch_documents')
    document_analysis = db.relationship('DocumentAnalysis', backref='batch_documents')